...
def getDiseaseInfo(keywords):
    results = DDGS().text(keywords, region='wt-wt', safesearch='Off', timelimit='y')
    # Iterate over the generator object to access its elements
        ...
        if currentState == 4:
            if '1' in user_message or 'disease' in user_message:
                try:
                    ...
                    response.append(f'<a href="https://www.google.com/search?q={disease_type} disease hospital near me" target="_blank">Search Near By Hospitals</a>')   
                    userSession[sessionId] = 10  # Transition to the next state
                except Exception as e:
                     # Handle any exceptions that might occur during prediction
                    response.append("An error occurred while predicting the disease. Please try again later.")
                    # Log the exception for debugging purposes
                    app.logger.error(f"Error predicting disease: {e}")
